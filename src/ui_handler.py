...
class UIHandler:
    ...
    def loading_spacer(message: str = "Processing...") -> Generator[None, None, None]:
        ...
        with console.status(
            f"[bold status.info]{message}[/bold status.info]", spinner="dots"
        ):
            yield
    ...
    def display_statistics(questions: List[Question]) -> None:
        ...
        for q in questions:
            ...
            color = (
                "green"
                if q.success_rate > 70
                else "yellow" if q.success_rate > 40 else "red"
            )
            ...
            table.add_row(
                ...
                q_type,
                str(q.times_shown),
                f"[{color}]{q.success_rate:.1f}%[/{color}]",
                q.text[:37] + "...",
            )
    ...
    def get_topic_input() -> str:
        ...
        return Prompt.ask(
            "\nEnter the [bold cyan]topic[/bold cyan] you want to study"
        ).strip()
    ...
    def get_question_id_input() -> str:
        ...
        return Prompt.ask(
            "\nEnter the [bold yellow]ID[/bold yellow] of the question"
        ).strip()
    ...
    def show_not_found(question_id: str) -> None:
        ...
        console.print(
            f"[bold red]Error:[/bold red] Question with ID '{question_id}' not found."
        )
    ...
    def validate_generated_questions(questions: List[Question]) -> List[Question]:
        ...
        for i, q in enumerate(questions, 1):
            console.print(
                Panel.fit(
                    f"[bold]Text:[/bold] {q.text}\n[bold]Answer:[/bold] {q.correct_answer}",
                    title=f"Question {i}",
                )
            )
            ...
            if choice == "a":
                ...
                q.correct_answer = Prompt.ask(
                    "Enter new answer", default=q.correct_answer
                )
                accepted_questions.append(q)
