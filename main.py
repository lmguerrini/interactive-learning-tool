    ...
    try:
        ...
        while True:
            ...
            if choice == '1':
                ...
                if topic:
                    with ui.loading_spacer(f"AI is crafting questions about '{topic}'..."):
                ...
                if isinstance(practice_q, MCQQuestion):
                    ...
                    with ui.loading_spacer("AI is evaluating your answer..."):
                        evaluation = quiz_manager.evaluate_freeform_with_llm(practice_q, user_ans)
                    if evaluation:
                        console.print(f"\n[bold]AI Judgment:[/bold] {'Correct' if evaluation.is_correct else 'Incorrect'}")
                        console.print(f"[italic]AI Explanation:[/italic] {evaluation.explanation}")
                        is_correct = evaluation.is_correct
                    else:
                        is_correct = False

                console.print("[bold green]Correct![/bold green]" if is_correct else f"[bold red]Incorrect.[/bold red] Reference: {practice_q.correct_answer}")
                ...
                for q in test_qs:
                    ...
                    if isinstance(q, MCQQuestion):
                        ...
                        with ui.loading_spacer("AI is evaluating..."):
                            eval_obj = quiz_manager.evaluate_freeform_with_llm(q, user_ans)
                        if eval_obj:
